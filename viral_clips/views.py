...
def transcribe_audio(request):
    """
    Start async transcription of an audio file using ElevenLabs API (Stage 2)
    
    This endpoint starts a Celery task to transcribe audio and returns a task_id
    for polling the status. Use /api/transcribe/status/<task_id>/ to check progress.
    ...
    try:
        ...
        # Start async task
        logger.info(f"Starting async transcription task {task_id} for {audio_url}")
        transcribe_audio_async.delay(task_id, audio_url, job_id)
...
@api_view(['GET'])
def transcribe_audio_status(request, task_id):
    """
    ...
    Returns: {
        "success": true,
        "status": "processing|completed|failed",
        "stage": "downloading|transcribing|uploading|complete",
        "percent": 50,
        "message": "Transcribing with ElevenLabs...",
